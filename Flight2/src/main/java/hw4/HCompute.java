 ...
 * Decode based on rowKeys format: AirlineID-Month-DateOfMonth-Year-FlightNum-Origin
 * @author csj
 *
 */
public class HCompute {
	...
	public static class HComputeMapper extends TableMapper<AirlineMonthPair, IntWritable> {
		...
		public void map(ImmutableBytesWritable row, Result value, Context context) throws IOException, InterruptedException {
			//rowKeys format: AirlineID-Month-DateOfMonth-Year-FlightNum-Origin
			String[] rowKeys = new String(value.getRow()).split("-");
			outKey.setAirline(rowKeys[0]);
			outKey.setMonth(rowKeys[1]);
